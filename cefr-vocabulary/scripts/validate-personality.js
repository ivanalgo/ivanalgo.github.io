...
const context={window:{CEFR_TOPICS:[{id:'personality',category:'Personal life',taxonomyReferences:['https://dictionary.cambridge.org/topics/people/words-used-to-describe-someones-personality/','https://dictionary.cambridge.org/topics/people/personal-qualities/']}],CEFR_ADDITIONAL_TOPIC_PACKS:{}}};
vm.runInNewContext(fs.readFileSync(path.join(root,'data/personality-b2-copy.js'),'utf8'),context);
const pack=context.window.CEFR_ADDITIONAL_TOPIC_PACKS.personality;
assert.equal(pack.vocabulary.length,50);
assert.equal(pack.scenes.length,5);
assert.equal(pack.idioms.length,4);
assert.equal(new Set(pack.vocabulary.map(item=>item.id)).size,50);
const examples=[];
...
if(process.argv.includes('--content-only')){
  console.log('Personality content OK: 50 words, 150 distinct bilingual examples, 150 bilingual collocations, 12 idiom examples.');
  process.exit(0);
}
...
console.log(`Personality OK: 50 words, 5 scenes, 55 distinct correctly sized images (${(total/1048576).toFixed(1)} MiB), complete bilingual copy.`);
