...
const manifest=require('../data/eating-out-image-prompts.json');
const root=path.join(__dirname,'..');
const context={window:{CEFR_TOPICS:[{id:'eating-out',category:'Food and drink',taxonomyReferences:['https://dictionary.cambridge.org/dictionary/english/a-la-carte-menu']}],CEFR_ADDITIONAL_TOPIC_PACKS:{}}};
vm.runInNewContext(fs.readFileSync(path.join(root,'data/eating-out-b2.js'),'utf8'),context);
const pack=context.window.CEFR_ADDITIONAL_TOPIC_PACKS['eating-out'];
...
for(const scene of manifest.scenes){
  ...
  scene.cards.forEach(([display],i)=>{
    const id='eating-out-'+slug(display),card=rendered.visualCards[i];
    assert.equal(card.id,id);
    assert.equal(pack.vocabulary.find(v=>v.id===id).display,display);
    assert.equal(card.image,`assets/images/eating-out/${scene.id}-cards/${slug(display)}.webp`);
    assets.push({file:card.image,width:768});
  });
}
if(process.argv.includes('--content-only')){
  console.log('Eating Out content OK: 50 words, 150 distinct bilingual examples, 150 bilingual collocations, 12 idiom examples; image mappings match.');
  process.exit(0);
}
...
console.log(`Eating Out OK: 50 words, 5 scenes, 55 distinct correctly sized images (${(total/1048576).toFixed(1)} MiB), complete bilingual copy.`);
