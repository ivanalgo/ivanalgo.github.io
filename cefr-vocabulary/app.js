...
function renderVisualWordCard(card){
  const item=byId(card.id);
  if(!item)return "";
  return `<article class="visual-word-card"><button class="visual-word-image" type="button" data-word="${item.id}" aria-label="Open ${escapeHtml(item.display)}"><img src="${card.image}" alt="${escapeHtml(card.alt)}" loading="lazy" width="800" height="600"></button><div class="visual-word-copy">${speakerButton(item)}<button class="visual-word-open" type="button" data-word="${item.id}"><b>${escapeHtml(item.display)}</b><span>${escapeHtml(item.sense.definitionZh)}</span><small>${escapeHtml(item.collocations[0])}</small></button></div></article>`;
}
function renderVisualPilot(scene){
  return `<section class="visual-pilot" aria-label="Small picture vocabulary cards"><div class="pilot-overview"><img src="${scene.image}" alt="Theme overview for ${escapeHtml(scene.title)}" loading="lazy"><div class="pilot-overview-caption"><span>Scene overview · 场景总览</span><b>先看整体语境，再逐词学习</b></div></div><div class="visual-word-grid">${scene.visualCards.map(renderVisualWordCard).join("")}</div></section>`;
}
function renderScene(scene){
  ...
  const visualContent=scene.visualCards?renderVisualPilot(scene):`<div class="scene-layout"><div class="visual ${scene.image?"":"icon-visual"}" style="--scene-accent:${scene.accent||activePack.topic.accent};--scene-accent-2:${scene.accent2||activePack.topic.accent2}">${sceneArt}${scene.labels.map(label=>`<button class="label label-${scene.id} label-${activePack.topic.id}" type="button" data-word="${label.id}" style="left:${label.x}%;top:${label.y}%">${escapeHtml(byId(label.id).display)}</button>`).join("")}</div><div class="word-list">${words.map(wordCard).join("")}</div></div>`;
  return `<article class="scene"><header><div><p class="eyebrow">${escapeHtml(scene.subtopic)}</p><h2>${escapeHtml(scene.title)} <span>${escapeHtml(scene.titleZh)}</span></h2><p>${escapeHtml(scene.description)}</p></div><b>${words.length} words</b></header>${visualContent}<section class="scene-collocations"><p class="eyebrow">Quick review</p><h3>Common collocations <span>常见搭配</span></h3><div class="bank-collocations">${collocations}</div></section></article>`;
}
